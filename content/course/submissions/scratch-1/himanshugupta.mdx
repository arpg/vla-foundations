...
## Loss Curve (when Causal Mask is ON)
...
## Attention Visualization (when Causal Mask is ON)

NOTE: The code to generate these plots was generated using the help of ChatGPT due to lack of familiarity with the process.
...
## Loss Curve (when Causal Mask is OFF)
...
#### Attention Visualization (when Causal Mask is OFF)
...
Based on these attention maps, we can see that when the causal mask is disabled, the attention matrices change significantly.
In this setting, tokens are no longer restricted to attending only to past positions and are free to attend to future tokens as well.
As a result, non-zero attention values appear in the **upper triangular region** of the attention matrices.

### Why the Model "Cheats"

When the causal mask is turned off, the model is able to attend to future tokens during training.
This allows each token to directly access the ground-truth token it is supposed to predict, or tokens that are highly correlated with it.
As a result, the prediction task becomes trivial.
Instead of learning to infer the next token from past context, the model can simply copy information from future positions through the attention mechanism. This leads to artificially low training loss, even though the model has not learned meaningful temporal relationships.
This behavior is often described as the model “cheating” because it exploits information that would not be available at inference time.
With the causal mask enabled, this shortcut is removed, forcing the model to rely only on past tokens and learn valid autoregressive behavior.

## Code Highlights

I enjoyed learning different ways of doing the same thing and understanding how each and every small component is implemented in the Transformer architecture.
