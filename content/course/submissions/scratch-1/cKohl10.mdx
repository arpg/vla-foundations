...
<figure>
  <img src="imgs/loss_curve_2.png" alt="Training Loss" />
  <figcaption>Figure 1: Training loss curve comparing RoPE and sinusoidal positional embeddings, showing RoPE converges faster to a lower final loss.</figcaption>
</figure>
...
RoPE treats the positional information of tokens as a complex conjugate pair of relative rotations. For example, the position $i$ is represented by the complex number $e^{i m \theta}$, and the position $j$ is represented by the complex number $e^{i n \theta}$, where $\theta$ is a non-zero constant defined from Vaswani et al. (2017) as $\theta = 10000^{-2(m-1) / d}$. The relative position $i - j$ is then represented by the complex number $e^{i (m - n) \theta}$. Practically, this is implemented in matrix form using the rotation matrix $\mathbf{R}_{\Phi, i-j}^{d}$, which is a block diagonal matrix with blocks of size $d/2$ that are rotation matrices for the relative positions.

## Attention Visualization

<figure>
  <img src="imgs/attention_layer_0_avg.png" alt="Attention Maps" />
  <figcaption>Figure 2: Average attention pattern across all heads in layer 0, showing strong diagonal focus indicating attention to recent tokens.</figcaption>
</figure>
...
<figure>
  <img src="imgs/attention_layer_2_heads.png" alt="Attention Heads" />
  <figcaption>Figure 3: Individual attention heads in layer 2, showing diverse strategies with some heads capturing longer-range dependencies and key sequence moments.</figcaption>
</figure>
...
When I removed the causal mask, the model is cheating and converges to a loss of ~0.5 after 6 epochs, indicating that future tokens can strongly inform the predictions of the model. We prefer not to use a model that does not have a causal mask because future tokens would not be known at inference time.

<figure>
  <img src="imgs/attention_layer_2_heads_no_mask.png" alt="Attention Map Cheating" />
  <figcaption>Figure 4: Attention heads in layer 2 without causal masking, showing sharper attention weights and continued focus on recent tokens despite access to future information.</figcaption>
</figure>

Surprisingly, I was expecting the attention heads to attend more broadly across the sequence without the causal mask, but the attention maps still show a strong focus on recent tokens. This suggests that even without the causal constraint, the model still finds it beneficial to prioritize recent context for making predictions. There is a noticibley stronger weight on each of the heads compared to the fuzzier results from the causal masked version. Since the model can leverage information from future tokens, it can make more confident predictions and thus focus its attention more sharply on the most relevant tokens.
