...
**Authors**: Anthony Hu*, Lloyd Russell*, Hudson Yeo*, Alex Kendall, Jamie Shotton, Zak Murez, George Fedoseev, Gianluca Corrado (*equal contributions)  
...
[PLACEHOLDER: Insert thesis statement here]
...
**Imitation Learning.** The advent of deep learning enabled a shift toward learning driving policies directly from human demonstrations. Kendall et al. [1] demonstrated that an end-to-end neural network could learn to drive in a day by imitating expert behavior. However, imitation learning suffers from distribution shift: the policy encounters states during deployment that differ from the training distribution, leading to compounding errors. Additionally, imitation learning cannot imagine scenarios beyond those present in the training data. It learns to mimic expert actions but not to reason about predicted futures or recover from novel situations.
...
1. **Representation Shift**: From pixel-level prediction to discrete token prediction
   ...
   - GAIA-1 uses VQ-VAE with DINO distillation for semantic tokenization
...
|-----------|------------|----------|
| **Image Tokenizer** | 0.3B | Discrete image encoding (VQ-VAE + DINO) |
...
The data flow proceeds as follows: (1) Input video frames are encoded into discrete image tokens via the tokenizer; (2) Text descriptions are embedded via frozen T5-Large; (3) Actions (speed, curvature) are projected via learned linear layers; (4) All modalities are interleaved into a single sequence and processed by the autoregressive world model to predict future image tokens; (5) Predicted tokens are decoded into video frames via the diffusion decoder.
...
The tokenizer incorporates inductive biases from DINO (self-supervised vision transformer) to ensure tokens are semantically meaningful rather than dominated by high-frequency signals.
...
$$
\mathcal{L}_{\text{video}} = \mathbb{E}_{\epsilon, t'} \left[ ||\epsilon_\theta(x_{t'}, t', z, m) - \epsilon||_2^2 \right]
$$

**Inference Pipeline**: Decode first 7 frames from tokens, autoregressively decode remaining frames (2-frame overlap), temporal upsample 6.25 Hz → 25 Hz, DDIM sampler with 50 steps.
...
**Information Decay.** The tokenizer compresses 3.5M bits → 7,488 bits (470×). Sub-pixel depth gradients, high-frequency textures, precise object boundaries, and small/distant objects may fall below tokenization resolution.
...
**Assumption 4: Generation Quality → Policy Performance.** No evidence that GAIA-1 generations improve downstream driving. Domain gap and generation artifacts may harm transfer.
...
**Proposed**: Closed-loop evaluation. Generate 1,000 hours of diverse scenarios with GAIA-1, train downstream policy on generated data, evaluate in CARLA/nuScenes (collision rate, route completion, comfort metrics), compare to real-data-only baseline. This would quantify the actual utility of world model generations for policy training.
...
**Authors**: Haoyu Zhen*, Qiao Sun*, Hongxin Zhang, Junyan Li, Siyuan Zhou, Yilun Du, Chuang Gan (*equal contributions)
...
**The 2D Representation Gap.** Prior video-based world models like UniPi, Genie, and Pandora generate future video frames conditioned on actions and text, but their 2D outputs suffer from several critical limitations for embodied tasks. First, they cannot provide metric depth information needed to plan precise grasping motions—a robot arm must know not just where an object appears in the image but how far it is and at what angle. Second, 2D models can produce geometrically inconsistent outputs where object sizes and shapes vary across frames, violating physical constraints. Third, converting 2D predictions back to 3D control signals requires additional perception modules (depth estimators, pose estimators) that introduce error accumulation and latency. The paper notes that "without accurate depth and 6-DoF pose estimations, robotic systems struggle to determine the exact position and orientation of objects."
...
TesserAct extends CogVideoX, a latent video diffusion model, to jointly predict RGB, depth, and normal videos. The architecture preserves the pretrained video generation capability while adding geometric prediction heads.
...
**Core Innovation**: Rather than predicting explicit 3D representations (meshes, point clouds, NeRFs), TesserAct predicts RGB-DN (RGB, Depth, Normal) videos as a compact 4D proxy.
...
**Training Objective**:
$$
\mathcal{L} = \mathbb{E}_{v_0, T, t, \epsilon} \left[ \| [\epsilon_v, \epsilon_d, \epsilon_n] - \epsilon_\theta(x_t, t, x_0, T) \|^2 \right]
$$
...
**Temporal Consistency Loss**: Frame-by-frame optimization lacks temporal coherence. TesserAct uses optical flow (RAFT) to enforce consistency:
...
$$
L_c = \lambda_{cd} \| \tilde{D}^i \circ M^i_d - D^{i \to (i-1)} \circ M^i_d \|^2 + \lambda_{cb} \| \tilde{D}^i \circ M^i_b - D^{i \to (i-1)} \circ M^i_b \|^2
$$

**Regularization Loss**: Prevents optimized depth from deviating too far from the generated prediction:
$$
L_r = \lambda_{rd} \| \tilde{D}^i \circ M^i_d - D^i \circ M^i_d \|^2 + \lambda_{rb} \| \tilde{D}^i \circ M^i_b - D^i \circ M^i_b \|^2
$$
...
**Synthetic Data (RLBench)**: 20 tasks × 1000 instances × 4 views = 80k videos with ground-truth depth. Normals estimated via depth2normal. Scene randomization (background, texture, lighting) via Colosseum pipeline.

**Real Data Annotation**: RollingDepth provides temporally consistent affine-invariant depth. Marigold-LCM-normal provides frame-consistent normal maps. This enables scaling to large real-world datasets without expensive sensor data.
...
- **Estimator quality**: Real-world depth/normal annotations depend on RollingDepth and Marigold quality, introducing systematic biases
...
3. Encode point cloud via PointNet
4. Concatenate with instruction language embedding
5. 4-layer MLP outputs 7-DoF actions

**Evaluation on RLBench** (success rate over 100 episodes):
...
**Depth Estimation Quality on Real Data**: Real-world depth annotations come from RollingDepth (affine-invariant, not metric). Converting to metric depth for control requires scale disambiguation, which may introduce errors. The paper doesn't report metric depth accuracy on real data.

**Inference Latency**: Video diffusion models are slow. TesserAct requires 50 DDPM sampling steps for 49-frame generation. The paper reports novel view synthesis in "~1 min" vs Shape of Motion's "~2 hours," but doesn't report end-to-end latency for closed-loop control. Real-time operation (10+ Hz) is likely infeasible.
