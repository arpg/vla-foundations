...
papers: "Genie, Cosmos, TesserAct, and GAIA-1"
---
...
The convolution on wavelet-space tokens separates Cosmos-Tokenizer1 from other standard architectures like VQ-VAE (CITE) by **removing redundancies** and **strictly maintaining a causal structure**.
...
The only other causal tokenizer that was compared to Cosmos-Tokenizer1 was the CogVideoX-Tokenizer (CITE) that was used in TesserAct, and Cosmos-Tokenizer1 outperformed it on the peak signal to noise (PSNR) reconstruction quality metric by ~3 points at comparable compression ratios when tested on the DAVIS dataset.
