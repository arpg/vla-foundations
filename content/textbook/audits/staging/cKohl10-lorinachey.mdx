...
$$
z_t = E_\theta(x_t) \in \left\{1, ..., K\right\}^{n}
$$
...
- Image tokens: $z_t \in \left\{1,...,8192\right\}^{576}$ (discrete VQ codes, learned embedding)
...
**Discrete Tokens vs. Continuous Latents.** GAIA-1 uses discrete tokenization ($z \in \left\{1,...,K\right\}^n$) rather than continuous latent representations.
...
- $z \in \left\{v,d,n\right\}$: Modalities (RGB, depth, normal)
...
$$
\epsilon^*_{d,n} = \text{DNProj}(h, \text{Conv3D}(\epsilon^*_v, [z_t; z_0]_{z \in \left\{v,d,n\right\}}))
$$
