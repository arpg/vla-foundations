...
Each world model analyzed in this document learns to predict future spatio-temporal states from a video sequence
(or, in TesserAct's case, a single image). All four models share a common three-stage structure: an encoder $\mathcal{E}$ compresses input frames
$x_{0:T}$ into latent tokens $z_{0:T}$; a dynamics model $\text{DYN}$ predicts future latent tokens $z_{T+1:T+K}$; and a decoder $\mathcal{D}$ 
renders the predicted frames $\hat{x}_{T+1:T+K}$:

$$\hat{x}_{T+1:T+K} = \mathcal{D}\!\left(\text{DYN}(z_{0:T})\right), \quad z_{0:T} = \mathcal{E}(x_{0:T})$$

The key architectural difference lies in how $\text{DYN}$ and $\mathcal{D}$ are implemented. **GAIA-1**, **Genie**, and **Cosmos's AR variant** 
implement $\text{DYN}$ as a fully autoregressive next-token predictor — each latent $z_{T+k}$ is conditioned on all prior latents $z_{0:T+k-1}$
 while the decoder $\mathcal{D}$ is a parallel diffusion model that renders pixel-space video from the predicted tokens.
 **TesserAct** and **Cosmos's diffusion variant** instead use a diffusion-based $\text{DYN}$ that denoises all future latents jointly.
 Pixel-space autoregressive decoding (predicting each output pixel sequentially) is avoided across all models: operating autoregressively
 in a compact latent space is already computationally intensive (576 sequential passes per frame for GAIA-1), and diffusion decoders produce
 higher-fidelity outputs by correcting quantization artifacts introduced during discrete tokenization.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><strong>TesserAct</strong></td>
      <td><strong>~5B parameters</strong> (fine-tuned from CogVideoX-5B)</td>
      <td><strong>~200,000 videos</strong> across RT-1, Bridge, and RLBench</td>
      <td><em>Training hardware not specified; inference requires 25–29 GB GPU memory (~30 GB model weights)</em></td>
    </tr>
  </tbody>
</table>
...
State of the art world models instead use a variety of discrete and continuous tokenization approaches. Discrete tokens map each latent
vector to a fixed entry in a learned codebook (e.g. via VQ-VAE), producing a symbolic integer index. Continuous tokens retain the full real-valued latent
vector output by the encoder without any quantization step, preserving finer-grained information at the cost of a larger representation space.

The models surveyed use the following approaches:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Models still struggle with <strong>physics adherence</strong> and object permanence in certain edge cases (e.g. teleporting through solid objects).</td>
...
*   **Modeling Physical Phenomena Beyond Classical Simulation:** Phenomena such as non-specular reflection, radiative diffusion, and granular media dynamics are practically impossible to encode faithfully in handcrafted simulators. Data-driven WFMs, trained on large-scale real video, can reconstruct these effects without explicit physics specification. This enables synthetic generation of rare physical edge cases at far higher frequency than they would be encountered in the world, providing a path toward building models that robustly understand phenomena that would otherwise be nearly unobservable in practice.
...
*   **Computational feasibility in the control loop**: The inference latency of diffusion and autoregressive WFMs makes real-time closed-loop operation with a VLA infeasible. Quantization or reduced diffusion sampling steps can force lower latency but collapse output diversity, producing a mean-seeking world model that under-represents the safety-critical edge cases a VLA must plan against.
...
### **5.3 Failure Modes**
*   **Information Decay**: The current **16-frame memory limit** causes long-horizon causal chains to fail, leading to object morphing or environment drift. This is a training-time sequence length constraint, not solely an inference limitation. 
Extending context via techniques such as mRoPE and long-context training is a viable architectural path forward, but would require retraining and substantially increases both training memory and inference latency, which is already a bottleneck at ~1 FPS.
