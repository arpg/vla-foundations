...
Multi-modality is the framework that enables a model to process and generate information across disparate data types; for VLMs, this is the gap between pixels and linguistic tokens.
The same principle extends beyond vision-language: audio, LiDAR, depth/point-cloud backbones, and scene-graph representations each contribute complementary signal that 2D RGB alone discards.

Before 2023, vision-language alignment relied on expensive, human-labeled datasets.
[LLaVA (2023)](#ref-llava-1) showed that a lightweight projection of [CLIP](#ref-clip) features into the LLM's token embedding space is sufficient: projected visual tokens are concatenated with language tokens and processed by the LLM's self-attention as a single sequence.

Since LLaVA, the landscape of Vision-Language Models (VLMs) has transitioned from modular bridging architectures to native multimodal/omni-architecture systems.
**LLaVA** introduced visual instruction tuning, mapping image features into the LLM's token space via a lightweight projection.
**[Prismatic](#ref-prismatic)** (the PRISM model family) later audited the design space and fused semantic ([SigLIP](#ref-siglip)) and geometric ([DINOv2](#ref-dinov2)) encoders to reduce information decay at the vision-language interface.
As of early 2026, **[Qwen3-VL](#ref-qwen3vl)** represents the frontier: it replaces the projection bridge with a unified multimodal token stream for text, images, and video, enabling 256K-native long context and an RL-trained "thinking mode."
...
The dominant recipe — supervised vision-language pretraining on human-curated caption and VQA corpora — required either expensive annotation pipelines or narrow domain-specific datasets.
LLaVA's contribution is to replace that bottleneck entirely: given a language-only GPT-4 and text descriptions of an image (COCO captions and bounding boxes), the model bootstraps multimodal instruction-following data **without any human labeling of the instruction set**.
The resulting **LLaVA-Instruct-158K** corpus became the template the modern VLM field inherits.

**The pipeline.** GPT-4 never sees the image.
...
### 2. Comparative Architectural Deep-Dive
...
$$\mathcal{L}_{\text{LLaVA}} = -\sum_{t} \log P_\phi(x_t \mid X_v, X_q, x_{<t})$$
...
**Mutual Information Bound.** We can bound the information carried by the visual tokens:

$$I(X_v; Z_v) \leq H(Z_v) \leq 576 \times 1024 \times \log_2(\text{precision})$$

The **effective information** is much lower: CLIP features lie on a low-dimensional manifold determined by the text-image matching objective, and the autoregressive LM loss does not recover what contrastive training threw away.
...
1. **Vision Encoder** $g(\cdot)$: Pre-trained CLIP ViT-L/14-336px (frozen; ~307M params)
2. **Projection Module** $W$: Linear layer (LLaVA 1.0) or 2-layer MLP ([LLaVA-1.5+](#ref-llava-15))
...
**Background: what is a "learnable token"?** In a transformer, tokens aren't strings — they're $d$-dimensional embedding vectors that enter the attention mechanism.
Every token in the model's vocabulary has a row in a learnable embedding table (a "dictionary" that maps token IDs to vectors); those rows are updated by gradient descent during training like any other parameter.
Text tokens index into a fixed vocabulary built from the training corpus; image patch tokens are produced on-the-fly by projecting pixel patches through a linear layer.
A *special* token like `[CLS]` is neither: it's a single learnable vector with its own entry in the embedding table, prepended to every input sequence as a placeholder that attention can "write into."

**The standard.** Transformer vision encoders (ViT, CLIP, DINO) prepend the `[CLS]` token at position 0 so it attends to every patch via self-attention, aggregating a global image-level summary.
That summary is exactly what single-vector tasks — classification, contrastive retrieval, linear probing — need.
In CLIP specifically, the contrastive image-text loss is computed on the `[CLS]` embedding, so the encoder is *trained* to compress image semantics into one vector.
...
### 3. The Scaling Frontier & Empirical Trends

*LLaVA is not a scaling-laws paper; this section stays light by design.
The paper's scaling-adjacent claims are architectural, not empirical-law.*
...
*LLaVA was not designed for robotic control; this section reframes LLaVA's engineering limits through a physicality lens, since those limits are exactly what propagates into downstream VLAs.*
...
### 5. Critical Synthesis & The "Load-Bearing" Audit
...
1.
**A language-only GPT-4 can generate *useful* multimodal instruction data from text descriptions alone.** The COCO captions + bounding-box serialization gives GPT-4 object identities, rough spatial layout, and counts — but not texture, material, lighting, or camera geometry.
...
2.
**A linear projection of CLIP features carries enough information for language-scale reasoning.** Bounded above by rank ≤ 576 (per the theorem in §2).
...
3.
**An autoregressive language-modeling loss is sufficient to align visual and linguistic representations.** True for chat-style VQA; less so for grounded tasks.
...
**Foundational.** Visual Instruction Tuning is the genuinely foundational move.
...
**POPE (Polling-based Object Probing Evaluation; Li et al., EMNLP 2023).** Probes object hallucination by asking the model yes/no questions of the form *"Is there a &lt;object&gt; in the image?"* A balanced set of present and absent objects is sampled under three splits that stress different failure modes:
...
**CHAIR (Caption Hallucination Assessment with Image Relevance; [Rohrbach et al., EMNLP 2018](#ref-chair)).** Measures how often a model mentions objects that aren't actually present, using COCO ground-truth object annotations as reference.
Two variants:
...
Both numbers are worse than InstructBLIP's — the Q-Former family holds up better on hallucination benchmarks, though the source of that advantage (Q-Former bottleneck? instruction data? hyperparameters?) isn't cleanly isolated in the literature.
...
The headline result — the **PRISM** model family outperforming LLaVA-1.5 across 12 VLM benchmarks (VQAv2, GQA, VizWiz, TextVQA, POPE, MM-Vet, MMBench, SEED, RefCOCO, OCID-Ref, VSR, AI2D) — is a byproduct; the real deliverable is the ablation itself.
...
|------|----------|-------------------------------|
| Optimization | Single-stage vs. multi-stage? Freeze or fine-tune the vision encoder? | Single-stage wins, saves 20–25% compute. Freeze wins on VLM benchmarks. |
...
The asymmetry between columns is instructive: DINOv2 alone beats SigLIP on localization but lags on VQA, confirming the semantic-vs-geometric split the fusion is designed to resolve.
...
Subsequent VLMs (including Qwen-VL family members) quietly dropped instruct-tuned LLMs from their recipes.
...
**Freeze wins.** Full fine-tuning significantly degrades performance, especially on spatial tasks (RefCOCO, OCID-Ref).
...
| Procedure | Stages | Notes |
|-----------|--------|-------|
| LLaVA-1.5 (multi-stage) | Stage 1: projector only on 558K captions · Stage 2: projector + LLM on 665K instruct | Vision encoder frozen throughout |
| Prismatic (single-stage) | Projector + LLM jointly on combined data | Vision encoder frozen throughout |
...
Combined overhead lands around **~350–400 GFLOPs**, dropping throughput on edge hardware (Jetson Orin) below **1.0 Hz** — well under the ~10 Hz typical robotic-control budget.
This is the engineering cost of buying the localization gains documented in RQ3, and it becomes a first-order constraint the moment the model has to close a control loop.
...
OpenVLA also ablates fine-tuning vs. freezing the vision encoder, and the finding **inverts Prismatic's RQ2**: fine-tuning the encoder is crucial for policy performance (**80.0% avg success** vs. **46.7% frozen**), with some frozen runs discontinued for near-zero success and unstable robot behavior.
...
Taken together the field now has three data points on the same axis.
Prismatic freezes (small VL data, text-output objective) and freezing wins.
OpenVLA fine-tunes (action-tokenization objective, ~970K [OpenX](#ref-openx) episodes) and fine-tuning wins.
Qwen3-VL fine-tunes (text objective at trillion-token scale) and fine-tuning is the unexamined default.
No single paper settles the question — there is no head-to-head controlled study that varies *only* data scale, or *only* objective, with everything else held fixed.
But the pattern is clear enough to state as a regime finding rather than a universal law: **freezing is a conclusion of a data-starved, text-output regime**.
Swap the objective to action generation and the recipe flips (OpenVLA).
Hold the objective fixed but scale the data by ~10³× and the recipe also flips (Qwen3-VL).
For a 2025/2026-era VLM project, Prismatic's RQ2 should be read as guidance bounded by its training-data scale — consistent with the field's post-Prismatic drift toward full fine-tuning at scale ([Qwen2-VL](#ref-qwen2vl), [Qwen2.5-VL](#ref-qwen25vl), [Qwen3-VL](#ref-qwen3vl)).

### 5. Critical Synthesis & The "Load-Bearing" Audit

#### Load-Bearing Assumptions

- **Static VLM benchmarks capture what matters.** The entire controlled sweep is adjudicated by VQA-style metrics on the 12-benchmark suite.
...
- **Patch-as-token is the right frame.** §3.1 of the Prismatic paper explicitly admits no cross-attention variant was tested.
...
- **Freezing the encoder generalizes.** The RQ2 finding is inverted downstream by OpenVLA.
Prismatic cannot distinguish an artifact of its data scale (explanation a) from a genuine property of VLM training (explanation b) — and the downstream evidence suggests the former.
...
Run a parallel design-space sweep that **(a)** extends the architectural-family axis to include at least one cross-attention variant (e.g., BLIP-2-style Q-Former) and one register-token variant (DINOv2 + registers), and **(b)** replaces the static-benchmark adjudicator with a downstream robotic evaluator (OpenVLA policy-success on BridgeData V2 plus a held-out sim environment such as SIMPLER or LIBERO).
This directly tests the two load-bearing assumptions the current paper cannot interrogate: does "patch-as-token" generalize across architectural families, and do VLM-benchmark winners survive the static → closed-loop transition?
The RQ2 inversion in OpenVLA Table 10 is the smoking gun — a follow-up sweep should make that inversion systematic rather than incidental.

## Part III. Qwen3-VL: Native Multimodality & Reasoning

### 1. Novelty & Contribution
As of early 2026, **Qwen3-VL** marks the arrival of **Native Multimodality**, treating images and videos as interleaved tokens in a massive **256K-token window**.
Its primary contribution is the **"Thinking Mode"**—a reasoning-enhanced edition trained via scaled reinforcement learning (RL) to perform deliberate, self-correcting reasoning before generating a response.
...
> **Audit implication:** “A22B active” is still enormous for real-time/edge control.
> Without explicit latency/FLOPs disclosure, deployment feasibility is unknown.
...
The "DeepStack-style" framing invites direct comparison with other information-injection patterns:

- **ResNet residual / DenseNet** — skip connections within a single backbone for gradient flow and feature reuse.
Same-signal, not cross-network.
- **U-Net** — encoder features piped to decoder at matched resolutions to preserve spatial detail.
Architecturally closest to DeepStack in spirit, but within one trained network.
- **LLaVA / prefix tokens** — visual tokens concatenated to the LLM input at layer 0 only; information propagates via self-attention through all $L$ layers.
- **Flamingo / cross-attention** — learned cross-attention layers interleaved every $k$ LLM blocks.
Expressive but pays full attention cost.
- **DeepStack** — projected vision features (from three vision-encoder levels) added directly to hidden states at three shallow LLM layers.
Cheaper than cross-attention, more than a single-point prefix.

The specific architectural claim is that multi-level vision features at multiple LLM depths, injected via projected residual add, is the cheap middle ground between LLaVA's single-point prefix and Flamingo's full cross-attention.
Structurally it is closest to U-Net's multi-resolution skip pattern, but operates between two separately-trained networks and the "resolutions" are encoder *depth* levels, not spatial resolutions.

**Innovation or marketing?** DeepStack-the-concept is from [DeepStack (Meng et al., 2024)](#ref-deepstack), not this paper.
The cross-layer-injection idea traces to skip connections generally and U-Net specifically.
What's specific to the Qwen3-VL instance is (a) three vision levels at the first three LLM layers, (b) projection-then-add rather than cross-attention.
This is real engineering, not a nothing — but the novelty is narrow: an architectural re-use and a hyperparameter choice, not an architectural invention.
The honest framing is "a specific, well-motivated instantiation of multi-level skip-injection at the VLM interface" rather than a new primitive.
The ablation question is handled in §4.3.
...
That’s ambitious.
Here’s where physical reality pushes back.
...
**Why:** The report demonstrates excellent long-context multimodal capabilities (256K native; ~1M extrapolated) and strong architectural ideas (interleaved positional encoding, cross-layer injection, textual timestamps).
But it does **not** provide the deployment-critical engineering disclosures (latency/FLOPs/token, token budgets vs resolution, alignment mechanics for DeepStack injection), and it doesn’t establish action grounding required for robotics-grade physicality.
