...
$$
\hat{a}_d = \left\lfloor \frac{a_d - \mathrm{lower}}{\Delta_d} \right\rfloor
$$
...
$$
\mathcal{L} = - \sum_{t} \log p(a_t \mid x, \mathrm{instruction}, a_{<t})
$$
...
$\pi_{0.5}$ jointly optimizes a cross-entropy next-token loss (L-CE) over text and FAST action tokens, alongside a flow-matching loss (L-FM) for the action expert:
