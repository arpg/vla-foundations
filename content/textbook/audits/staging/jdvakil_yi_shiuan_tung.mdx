...
$\pi_{0.5}$ jointly optimizes a cross-entropy next-token loss ($\mathcal{L}_{\mathrm{CE}}$) over text and FAST action tokens, alongside a flow-matching loss ($\mathcal{L}_{\mathrm{FM}}$) for the action expert:
