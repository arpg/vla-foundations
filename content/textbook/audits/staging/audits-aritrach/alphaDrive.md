...
The paper argues that naive “correctness reward” used in math/programming does not transfer cleanly to planning; you need a reward that is robust early in training and resistant to shortcut solutions.

### 2.2 Context

- **End-to-end driving models** can output trajectories/controls directly from sensors, but the paper positions them as “black-box” systems that struggle with long-tailed driving cases because they lack explicit commonsense/reasoning.
...
3. **RL-augmented VLM planners (AlphaDrive’s focus)**
   - Use RL to push beyond imitation and learn better decision policies.
...
- **Navigation**: derived from sparse navigation points (AMap / Google Maps-like) and converted into text (e.g., “Go straight for 100m, then turn right”).
- **Output format**: reasoning inside `<think>` and final answer inside `<answer>` tags; non-conforming outputs receive **format reward = 0** (hard penalty).

### 3.2 Base Model Choice

They use **Qwen2VL-2B** as the base model, motivated by:

- better meeting latency requirements than larger variants, and
- better support for RL training (as claimed).
...
**Audit note:** GRPO is a reasonable choice *if* your reward truly ranks “better plans” in a meaningful way. That pushes the entire paper’s load-bearing weight onto reward design.

---

### 4.2 Planning Reward Modeling (the key contribution)

AlphaDrive introduces **four rewards**, then combines them into the final RL signal.

#### Reward 1 — Planning Accuracy Reward (F1, not exact match)

They found exact-match reward unstable early (format noise like case sensitivity/extraneous tokens), and “GT included among words” encourages a shortcut (“output all possible actions”), causing collapse. They adopt **F1-score** for lateral and longitudinal decisions separately for stability and shortcut resistance.
...
#### Reward 3 — Planning Diversity Reward (anti-collapse)
...
**Industry critique (balanced):**

- **Strong engineering:** they explicitly test and reject reward designs that cause reward hacking (“output everything”) and early instability, then choose F1 for robustness.
- **Risk:** multiplicative reward can create “dead zones” (one factor collapses → no gradient signal). In production, you’d test whether rare-but-critical cases get undertrained.
...
They adopt **MetaAD** as the benchmark:
...
**Audit note:** BLEU/CIDEr/METEOR measure surface similarity, not causal correctness. For a safety domain, you’d want attribution correctness tests.
...
- The strongest listed fine-tuned baseline Qwen2VL† (7B) reports **61.44** accuracy.
...
**Audit interpretation:** The “headline” isn’t just that RL helps — it’s that **SFT collapses faster under reduced data**, while RL and especially SFT+RL retain performance better. This is exactly the kind of result industry likes because labeled driving data is expensive.
...
- **Con:** shifts risk to the interface between **symbolic plan → downstream controller**. You now must prove that the downstream stack can robustly interpret “decelerate, left” in dense traffic.
...
**Audit question:** Does MetaAD include enough “slow-burn” causal chains (construction ahead, multi-step merges), or is it mostly short reactive planning? The paper doesn’t resolve this; you’d need dataset slicing.
...
3. **Reasoning usefulness assumption**
   Distilled reasoning is assumed to improve decisions, not merely produce nicer explanations; they explicitly found RL-only reasoning to be messy/weakly causal and patched it by teacher-generated traces.
...
**Gaps (from an industry standpoint):**
...
2. GRPO objective and motivation in AlphaDrive.
