...
paper: "EMMA, AlphaDrive, Alpamayo-R1"
...
| Model | Primary Inputs | History Window | Framerate / Temporal Context | Primary Outputs | What “Action” is |
|---|---|---|---|---|---|
| **AlphaDrive** | Single front-view image + text prompt (current speed + navigation instruction) | **None** — single-frame inference; no temporal history window | Not specified; one image per inference step | **Meta-actions** (lateral + longitudinal categories) and optionally structured reasoning | **Discrete high-level driving decision** (category-level) |
| **EMMA** | Surround-view camera images (6–8 cameras, 360°) + intent command + **ego-waypoint history as plain-text BEV coordinate pairs** $T_\text{ego} = \{(x_t, y_t)\}_{t=-T_h}^{-1}$ + task prompt | **2 seconds** of historical ego waypoints | ~2 Hz on nuScenes (standard protocol → ~4 historical frames); internal Waymo dataset rate not disclosed | **Waypoints/trajectories as text**, plus detection + road-graph outputs depending on prompt | **Trajectory as language** (BEV coordinates emitted as plain text at 2 decimal place precision) |
| **Alpamayo-R1** | 2 front-facing cameras (120° wide-angle + 30° telephoto; 7-camera surround supported) + egomotion history + optional text commands | **2 seconds** of historical egomotion data | 2 Hz auto-labeling sampling rate → ~4 frames of history in context | **Structured reasoning + discrete trajectory tokens**, then **continuous trajectories via flow-matching decoder** | **Multimodal continuous trajectory**, efficiently decoded from tokens |

---

# Training Data

| Model | Dataset | Volume | Construction | Implicit Emphasis |
|---|---|---|---|---|
| **AlphaDrive** | MetaAD (proprietary, not publicly available) | 120k clips × 3s (~100 hours equivalent); 110k train / 10k val | Unknown: no mention of whether data comes from human drivers or a robotic/AV platform; "balanced distribution" stated as a property with no methodology described; geographic distribution not disclosed | Short-horizon, discrete meta-action supervision; distribution and any collection biases unknown |
| **EMMA** | Public: nuScenes (6 hrs / 18k), WOMD (572 hrs / 487k); Internal: **203,117 hours / 24,374,046 examples** from Waymo fleet | Internal dataset is ~355× WOMD; largest disclosed driving corpus in this audit | Self-supervised from logged ego trajectories — no human annotation required; Waymo fleet data from real-world deployments | Expert human driving decisions from Waymo's urban US/international markets; imitation learning encodes driver choices as ground truth; data scale is the primary driver of EMMA+ SOTA |
| **Alpamayo-R1** | NVIDIA internal fleet (80,000 hrs, 2,500+ cities, 25 countries) + CoC reasoning corpus (700K segments via hybrid auto-labeling + human-in-the-loop) | 80k hours driving data; 700K CoC video segments with structured reasoning traces | Auto-labeling (GPT-5, rule-based kinematic detectors) + ~10% human-labeled with two-stage QA; geo-fenced evaluation (held-out geographies from training) | Long-tail generalization across diverse geographies and scenarios; CoC traces emphasize causal decision factors; hybrid labeling prioritizes accuracy over scale |

---

# Training & Supervision

| Model | Training Stages | Trajectory Objective | What the objective emphasizes |
|---|---|---|---|
| **AlphaDrive** | (1) Distill reasoning from a larger teacher → **SFT** warm-start; (2) **GRPO RL** refinement | **None** — outputs discrete meta-actions (no trajectory); supervised by GT meta-action labels via F1-score + GRPO reward shaping | **Multimodal planning** (diversity), **safety-critical weighting**, and structured output constraints |
| **EMMA** | Multitask training with a unified language formulation; adds **CoT** prompting/training | **Autoregressive next-token prediction** over text-encoded BEV coordinate pairs $\{(x_t, y_t)\}$ (2 decimal places); supervised by future ego locations from logs (self-supervised, no labels) | **Shared interface across tasks**; co-training yields cross-task gains; objective is identical to language modeling — coordinates treated as word tokens |
| **Alpamayo-R1** | Multi-stage: add action modality → SFT for reasoning → **RL post-training**; plus **CoC dataset/pipeline** | **128 discrete trajectory tokens** (64 waypoints × \{acceleration $a^i$, curvature $\kappa^i$\} via unicycle dynamics); cross-entropy loss during training; flow-matching action-expert decodes discrete embeddings to continuous trajectories at inference | **Causal structure**, **reasoning/action consistency**, and kinematically feasible multimodal trajectories under runtime constraints; crucially distinct from EMMA's text encoding — acceleration + curvature respect unicycle kinematics by construction |
...
**Open-loop evaluation** replays recorded driving logs and scores predicted trajectories against ground-truth human behavior — no feedback, no interaction; AlphaDrive and EMMA are evaluated exclusively open-loop.
**Closed-loop evaluation** places the model in a simulation loop where its actions alter the scene state and it must respond to the consequences — testing compounding errors and interactive behavior; Alpamayo-R1 is the only paper here with closed-loop evaluation (via AlpaSim).
...
On MetaAD, AlphaDrive (2B) achieves 77.12% planning accuracy, a 25.5% relative gain over the fine-tuned Qwen2VL-7B (61.44%), the strongest baseline despite being 3.5× larger. 
Even still, this means that one quarter of the time the selected meta action is incorrect to the ground truth label.
The data-efficiency case is notable: at only 20k training samples, the SFT+RL strategy outperforms SFT-only by 35.31% (55.64 vs 41.12%), suggesting that RL's value is most pronounced in low-data regimes where SFT alone tends to overfit.
---
...
### 2.2 Context
- Existing **End-to-end driving models** can output trajectories/controls directly from sensors, but they are “black-box” systems that struggle with the long-tail of driving cases because they lack explicit reasoning.
- **VLM-based planners** shift some of that burden: use vision + language prompting to decide higher-level actions, which can incorporate “commonsense” reasoning.
The paper provides an example prompt where the model is asked to plan for the next three seconds using a speed + navigation command.
...
### 3.1 AlphaDrive Deep Dive Example
AlphaDrive generates multiple candidate answers per scenario (here, two), each containing a reasoning trace in `<think>` tags and a meta-action decision in `<answer>` tags. 
These candidates are scored across four reward dimensions: planning accuracy (F1 against the ground-truth action), action-weighting (higher weight for safety-critical behaviors like braking), planning diversity (penalizes identical outputs across candidates), and format compliance (binary check on tag structure). 
The first three are multiplied together into a planning quality score computed separately for speed and direction, then combined with the format reward to compute the GRPO loss. 
Because GRPO optimizes relative advantages within the group of candidates, the model learns to favor higher-scoring answers over lower-scoring ones rather than optimizing against an absolute threshold — which the authors argue is a natural fit for planning, where multiple valid solutions may exist.

The Figure 2 example raises several concerns. 
The ground-truth is "accelerate, left_turn," but the navigation command says "go straight" — a contradiction the paper doesn't explain. 
Answer1 reasons defensively about pedestrians preparing to cross and outputs "keep, straight," a plausible human decision that receives zero planning accuracy reward. 
Because accuracy multiplies with the other rewards, this cautious yield scores identically to nonsense, directly undermining the diversity reward's stated purpose of encouraging multiple valid solutions. 
The conversational "User" prompt framing — "what is your driving plan for the next three seconds?" — also reveals an architectural mismatch: this is a chat-style VQA system, not a continuous control loop. 
The 3-second planning horizon matters here, because Answer1's pedestrian reasoning could be momentarily correct but stale by execution time, and the paper provides no indication of inference frequency or how the system handles rapidly evolving scenes where overyielding becomes a real safety and traffic-flow concern.
...
AlphaDrive uses **Group Relative Policy Optimization (GRPO)**.
The paper defines GRPO as:
...
They used 30,000 clips from the 110,000 clips in the training set to generate example reasoning traces.
---
...
**Critical reproducibility gaps:** The paper provides essentially no methodology for how MetaAD was collected or structured.
It does not state whether the data comes from human drivers or a robotic/AV platform — a fundamental question for understanding what behavior is being imitated.
It does not describe how "balanced distribution across environments and planning actions" was achieved: no mention of sampling strategy, frequency of rare events, or whether balance was enforced during collection or post-hoc via resampling.
Geographic distribution, sensor configuration, and collection dates are not disclosed.
"Well-balanced distribution" is stated as a property of the dataset, not described as a procedure — making it impossible to evaluate whether this claim holds or how to reproduce it.
...
<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/alphadrive/alphadrive_vis.png" alt="AlphaDrive results visualization" width="900" />
...
- **Con:** shifts risk to the interface between **symbolic plan → downstream controller**.
Need to prove that the downstream stack can **robustly** interpret “decelerate, left” in dense traffic.
...
1. **Reward alignment assumption**
   ...
   Weighting safety actions (those requiring speed changes) higher without grounding the driving decision in reasoning, leads to vehicle that uses velocity adjustment as the primary factor for safety generation.
...
- **Data leakage concern:** MetaAD serves as both the SFT training corpus (110k clips) and the evaluation benchmark (10k clips).
The paper states: “We adopt MetaAD, a large-scale real-world driving dataset, as our training and evaluation benchmark.”
The 10k validation clips are geo-fenced from the 110k training clips, but both come from the same undisclosed distribution.
No evaluation on any external, independently collected benchmark is reported — so the 77.12% accuracy figure is entirely in-distribution and provides no evidence of generalization.
...
2. **Diversity reward producing “diverse but unsafe” plans**
   Diversity is rewarded by penalizing same meta-driving actions among sampled answers.
   - Risk: incentivize meta-action disagreement without regard to scene characteristics.
   - Risk: more meta-action options encoded during training, however, the right driving decision becomes muddled.

3. **Format-induced brittleness**
   Format reward is hard-zero when tags fail.
   - Risk: rare formatting drift can be catastrophic in a production parser unless you robustify extraction (fuzzy parsing, constrained decoding, default safe fallback).
...
- **Sign off for research adoption:** Conditional Yes — Need to rerun all findings on open source dataset o confirm strong evidence that tailored RL (GRPO) + planning reward engineering + reasoning distillation improves a high-level VLM planner and yields better data-efficiency.
...
The core thesis is that **large-scale multimodal foundation models carry world knowledge and reasoning capabilities that specialized modular autonomy stacks cannot replicate**, and that recasting driving tasks as VQA problems is sufficient to unlock those capabilities without specialized components. 
EMMA fine-tunes Gemini with self-supervised trajectory objectives (requiring only future ego locations, no human labels), optionally augmented with **chain-of-thought (CoT) reasoning** that produces a hierarchical driving rationale before outputting waypoints. 
The generalist variant co-trains across planning, 3D detection, and road graph estimation via instruction tuning.

On nuScenes, EMMA achieves **state-of-the-art open-loop planning**, outperforming prior supervised methods by 6.4% and self-supervised methods by 17.1% in average L2. 
On Waymo Motion Dataset, EMMA+ (EMMA trained on a 24 million example internal dataset) with Chain of Thought (CoT) reasoning surpasses MotionLM and Wayformer by **13.5% at the 5-second horizon**. 
...
Recent work \[6,12\] revealed that strong benchmark performance on prior end-to-end models could be achieved by simply extrapolating historical ego status, raising questions about genuine scene understanding. 
EMMA addresses this by ablating Gemini pre-trained weights (versus random initialization) and showing that CoT scene reasoning substantively improves trajectories beyond what ego-status extrapolation supports. 
Unlike **VLM-augmented driving planners** (DriveGPT4, DriveVLM, OmniDrive\[7\]) that bolt an LLM onto a conventional autonomy stack as a post-hoc explainer, EMMA treats the MLLM as a first-class citizen that directly makes the driving decisions.
...
- **Vision input ($V$):** Surround-view camera images (6–8 cameras, 360° coverage).
...
**Key design choice — text over special tokens:** 
EMMA uses direct floating-point text for coordinates rather than learned discretization (compared with MotionLM)\[4\]. 
This maximally reuses pre-trained weights at the cost of more output tokens per coordinate.

<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/emma/emma-architecture.png" alt="Figure 1: EMMA Overview Diagram" width="900" />
...
No specialized driving components are added — the entire model is fine-tuned from Gemini weights using task-specific prompt-answer pairs from driving logs. 
Training hardware is not disclosed; the internal dataset scale implies industrial-level compute that makes external reproduction impractical.
...
This scale advantage carries an implicit cost: using logged trajectories as supervision assumes the human driver made the correct decision in every scenario, encoding the biases, errors, and edge-case failures of the fleet operator directly into the training signal.
Waymo has the largest disclosed corpus of human expert driving data of any company in this audit: 203,117 hours and 24,374,046 training examples, approximately 355× the size of the publicly available WOMD dataset (572 hours / 487k examples) — meaning the primary driver of EMMA+'s SOTA performance is, in large part, access to an unmatched volume of proprietary fleet data that is unavailable to external researchers.
...
This is a known failure mode of behavior cloning and is explicitly acknowledged as an unresolved limitation.
...
Rationale captions are generated automatically using off-the-shelf Waymo perception/prediction models and Gemini prompting so no additional human labels required. 
Prediction order of rationale versus trajectory does not significantly affect quality post-convergence, allowing early stopping on trajectory tokens at inference time.

<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/emma/emma-cotraining.png" alt="Table 1: CoT Reasoning Ablation" width="900" />
*Table 1: Ablation on chain-of-thought components. 
Meta decision and critical object identification contribute the largest gains (+3.0% and +1.5%). 
Full CoT improves planning by +6.7%.*

### 4.3 Generalist Multi-Task Training

Three task families are co-trained via instruction tuning with task-specific prompts: **3D object detection** (7D bounding boxes sorted by depth), **road graph estimation** (lane polylines as BEV waypoints), and **scene understanding** (binary blockage detection). 
During each training iteration, batches are drawn from all task datasets simultaneously, with each dataset sampled at a frequency proportional to its size — so larger datasets like the 24M-sequence motion planning set naturally dominate training. 
Key road graph design choices — dynamic curvature-based waypoint sampling, ego-origin alignment, shuffled ordering, and padding — contribute **70–90% performance differences** on lane-level metrics, with language-like punctuation adding another ~10%.

<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/emma/generalist-emma.png" alt="Figure 2: EMMA Generalist Illustration" width="900" />
...
The internal motion planning dataset is ~355× larger than WOMD, comprising 30-second real-world clips sampled at one frame per scenario. 
This scale is the primary driver of EMMA+ performance.

### 5.2 Evaluation Metrics

- **Planning:** L2 displacement error (meters) at 1s, 2s/3s, and 5s horizons.
Multiple trajectory sampling used on WOMD (Top-K up to K=24, selecting the "median" trajectory); single top-1 on nuScenes.
...
- **Scene Understanding:** Accuracy versus human baseline on blockage detection.

### 5.3 Main Performance Results

**nuScenes** — EMMA: **0.32m avg L2** (self-supervised), outperforming all prior self-supervised methods (BEV-Planner: 0.35m)\[6\] and supervised methods (OmniDrive: 0.33m, despite HD-map labels). 
EMMA (random init): 0.37m, confirming Gemini pre-training weights drive the advantage.
EMMA+: **0.29m**.

<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/emma/nuscenes-results.png" alt="Table 2: nuScenes Planning Results" width="900" />
...
This result strengthened the case that camera-only end-to-end models could match or exceed systems built on LiDAR-derived structured inputs, though the comparison is on an internal benchmark and the baselines differ in input modality rather than being direct sensor-level comparisons.

<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/emma/womd-results.png" alt="Table 3: WOMD Planning Results" width="900" />
...
<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/emma/emma_improved_detection.png" alt="Figure 3: 3D Detection Precision-Recall Curves" width="900" />
*Figure 3: Camera-primary 3D detection on WOD using LET matching. 
EMMA+ achieves 16.3% relative improvement in vehicle precision at the same recall versus BEVFormer.*
...
<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/emma/emma-data-scaling.png" alt="Figure 4: Data Scaling Curves" width="900" />
*Figure 4: Eval perplexity for end-to-end planning versus training FLOPs at four dataset sizes (3%–100% of internal 24M dataset). 
Performance has not saturated at full scale.*

**Co-training gains:** Detection + planning: +2.4% detection, +1.6% planning. 
Road graph + planning: +3.5% road graph, +1.4% planning. 
All three together: **+5.5% detection, +2.4% road graph, +1.4% planning** versus single-task specialists. 
Planning is the dominant anchor — detection + road graph without planning yields −1.4% planning.


<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/emma/emma-cotraining-experiments.png" alt="Table 4: Generalist Co-Training Results" width="900" />
*Table 4: Co-training across all three tasks yields the best results, outperforming single-task specialist models by up to +5.5%. 
Planning is the dominant anchor task — co-training detection and road graph without it yields −1.4% planning performance.*
...
EMMA outputs continuous BEV waypoints as plain-text floating-point pairs discretized to two decimal places, an inherent quantization cost of the unified language representation.
Beyond precision loss, the text representation introduces two additional degradation mechanisms.
First, LLM tokenizers handle numbers inconsistently: coordinate strings like "1.99" and "2.00" may map to different token sequences despite representing nearly identical spatial positions, creating discontinuities in the embedding space at arbitrary numeric boundaries.
Second, the geometric structure of a trajectory (e.g., that waypoint 3 lies spatially between waypoints 2 and 4, or that a sequence curves left) must be inferred entirely from the linear token sequence, with no structural inductive bias to assist the model.
The paper notes "the text representation may produce more tokens than specialized tokenization" but does not analyze when these failure modes occur.
More critically, EMMA is **camera-only**: detection performance degrades sharply beyond 30m (F1: 0.93 at 0–30m versus 0.73 at 50m+), precisely where objects must be detected earliest at highway speeds. 
The tradeoff is generalizability and simplicity versus inherent depth precision.

### 6.2 Benchmark Critique

The paper explicitly acknowledges **nuScenes' limitations**: collision-rate metrics are sensitive to BEV grid resolution, and many scenarios can be trivially solved by ego-status extrapolation. 
The random-init ablation partially addresses this, but the more fundamental issue is that all evaluation is **open-loop** — predictions are scored against logged ground-truth without interactive feedback. 
Open-loop performance is a necessary but not sufficient condition for safe deployment. 
The paper identifies NAVSIM\[10\] and realistic sensor simulation as potential paths forward.

### 6.3 "Generalist World Model" Claim

EMMA shows a co-trained single model can match or surpass specialist models across three tasks simultaneously, suggesting the joint visual representation encodes a more complete scene model. 
However, the paper concedes there is "**no guarantee that these outputs will be always consistent.**" 
When trajectory and detection outputs contradict each other (for example, stopping for an agent that was never detected) there is no way to verify the model's reasoning. 
The paper acknowledges this risk but provides no metric to measure how often it occurs.
...
1. **Pre-training transfer is the performance driver, not scale alone.** 
EMMA's gains could come from Gemini's world knowledge or its 24M proprietary dataset. 
The paper tests them together, so their individual contributions can't be separated.

2. **Text coordinate representation is sufficient for safe planning.**
Waypoints are generated token-by-token like prose. 
Small systematic errors may average out in L2 but still cause dangerous local failures (e.g. a stopping line).
In retrospect, this reads as the expedient choice of a first-mover: EMMA was the first end-to-end VLM for driving, and direct text conversion of coordinates maximally reused pre-trained Gemini weights without specialized engineering.
The failure modes are concrete — precision is capped at two decimal places, LLM tokenizers handle numbers inconsistently (e.g., "1.99" and "2.00" may map to different token sequences despite near-identical values), and the spatial structure of a trajectory must be inferred entirely from the token sequence with no structural inductive bias.
Alpamayo-R1, released approximately a year later, replaces text-encoded coordinates with a control-based tokenization scheme (128 discrete tokens encoding acceleration and curvature via unicycle dynamics), achieving both better trajectory quality and faster inference — validating this critique empirically.

3. **Self-supervised trajectory labels are sufficient to learn perception.**
Only trained on ego waypoints, no perception labels. 
The model may be pattern-matching scenes rather than genuinely reconstructing them geometrically.

4. **Open-loop performance transfers to closed-loop behavior.**
The broadest and most critical assumption, entirely unvalidated empirically.
...
**Gaps:** Internal Waymo 24M-sequence dataset unavailable externally — EMMA+ is unreproducible. 
Gemini weights are proprietary **(PaLI-X variant non-trivially underperforms: 0.680m versus 0.543m at WOMD 5s)**. 
Inference latency and memory footprint not systematically reported. 
Multi-task output consistency not measured despite acknowledged risk.
...
5. **Closed-loop error compounding.**
The central known risk of behavior cloning: systematic trajectory biases accumulate into out-of-distribution states not seen during training.
This is a well-known, common limitation of imitation learning and not unique to EMMA.
Data scale is a documented mitigation: EMMA's scaling curve (Fig. 4) shows monotone improvement from 3% to 100% of the 24M-example dataset with no saturation, meaning continued data collection directly reduces the distribution gap.
This should be treated as a known, manageable engineering challenge rather than a fundamental architectural risk.

### 7.4 The Next 10,000 GPU-hour Experiment

**Experiment A — "Closed-Loop Stability via Sensor Simulation"** <br />
Leverage neural scene reconstruction (like NeRF, Gaussian Splatting, etc.) to generate novel camera observations from perturbed ego states, enabling reactive closed-loop evaluation without a full physical simulator.
...
Define a consistency oracle: if a bounding box is predicted at $(x, y, \theta)$, the trajectory should respect that agent's right-of-way; if the road graph predicts a merge in 40m, the trajectory should respond within that horizon.
Run on the full nuScenes validation set.
- **Metrics**: Consistency failure rate for safety-critical classes, or inconsistencies used to build a consistency-regularized training objective.

### 7.5 Sign-Off Criteria

- **Sign off for research adoption:** Yes — strong evidence that MLLM fine-tuning with self-supervised trajectory objectives achieves state-of-the-art open-loop planning while improving perception tasks. 
Co-training results and CoT ablations support genuine scene understanding. 
Unsaturated scaling curves indicate continued ROI on data and compute.

- **Sign off for production readiness:** 
Conditional No — missing closed-loop validation, no LiDAR integration, no formal multi-task consistency guarantee, and non-trivial inference latency (3 FPS optimized versus ~20 FPS required). 
The path exists but requires: (1) closed-loop evaluation infrastructure, (2) LiDAR/radar fusion, (3) latency optimization, (4) formal consistency verification.
...
**Generalization** targets allowing composition of causal factors across data from 25 countries and 2,500 cities to make driving decision environments with minimal or no training data.
...
Chain of Thought (CoT) differs from Chain of Causation because the components that CoT sometimes focuses on aren't directly linked to the produced driving decision.
This can lead to lots of additional context being included in the model, increasing inference time and reducing driving meta action accuracy.
...
In open-loop evaluation, yielding behavior at a four-way stop is scored against the recorded driver's decision — a small timing deviation produces a small L2 penalty and nothing more. 
In closed-loop, that same timing deviation shifts the ego vehicle's position, which changes the sensor observations, which changes the next prediction, creating a feedback loop the model was never explicitly trained to handle. 
The model must now recover from a state of its own making rather than simply matching a pre-recorded trajectory.
...
2) **Causal understanding gap**  
   ...
   - vague (“be cautious”),
   - manuever fails to be grounded in decision-committing,
...
#### Outputs
- **CoC reasoning trace**  
  A structured explanation aligned to a **closed-set driving decision (longitudinal/speed and lateral/steering)** that is anchored to an *explicit* decision category.
...
<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/alpamayo/alpamayo_main_arch.png" alt="Alpamayo Architecture" width="900" />
...
- *6.4-second future trajectory*  
  ...
  The 6.4s refers to the **prediction horizon** — how far ahead the trajectory extends — not inference latency.
  This horizon is 64 waypoints sampled at 10 Hz ($64 \times 0.1\text{s} = 6.4\text{s}$), long enough for a vehicle at highway speeds to plan ~190m ahead.
  End-to-end inference latency is separately measured at **~99ms** on an NVIDIA RTX 6000 Pro Blackwell (vision encoder prefill: 3.43ms, LLM prefill: 16.54ms, reasoning decode: ~70ms, trajectory decode: 8.75ms), a completely different quantity from the planning horizon.
...
AR1 is explicitly staged rather than “train everything end-to-end once.”
The motivation is that they want:
- strong perception + physical priors,
- controllable action decoding,
- then structured reasoning,
- then alignment.

<img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Zaaler-aritrach-autonomous-driving/content/textbook/audits/staging/figures/alpamayo/alpamayo_train_pipeline.png" alt="Alpamayo Training Pipeline" width="900" />

### 4.1 GRPO as the RL Backbone

For post-training, the paper uses a **GRPO-style** (Group Relative Policy Optimization) approach:

- Sample **K = 6 rollouts** per prompt/context (stated explicitly throughout the evaluation sections).
- Score them with reward models / critics.
- Use relative advantages within the group to stabilize learning.
- Apply KL regularization **every iteration** (λ_KL coefficient in the loss term) against π_ref (the Stage 2 SFT model).

**Compute cost**: Sampling K = 6 rollouts per policy update multiplies per-step compute by **6×** relative to standard SFT — each update requires 6 full VLM forward passes plus 6 reward evaluations before a gradient step.
To mitigate this, AR1 performs RL on a curated high-disagreement subset (samples where the model’s implicit reward disagrees with the explicit reward signal) rather than the full dataset.

**Number of reward critics**: 1 LRM (DeepSeek-R1 or Cosmos-Reason) for reasoning quality (r_reason); the consistency reward (r_consistency) and trajectory quality reward (r_traj) are **rule-based** and require no separate neural critic.
...
This is the **LLM as a judge** paradigm — the paper explicitly uses this framing, with **DeepSeek-R1** or **Cosmos-Reason** as the evaluator model.
The rubric scores from 0 (completely unrelated or opposite to GT) to 5 (behavior and causal reasoning fully consistent with GT CoC trace), providing a continuous scalar reward signal rather than a binary pass/fail.
...
This is **not** direct text comparison between the CoC trace and the action tokens — it uses two separate rule-based pipelines that produce comparable labels:

**Step 1 — Trajectory → meta-action labels (rule-based):**
The predicted trajectory $\tau$ is passed through kinematic rule-based detectors (the same detectors used in auto-labeling, Table 5) that output atomic meta-actions encoding longitudinal behavior (e.g., gentle accelerate, strong decelerate) and lateral behavior (e.g., steer left, go straight).

**Step 2 — CoC reasoning → intent labels (rule-based parsing):**
The generated reasoning trace is parsed to infer the intended driving decision against the closed decision set (Table 1: 7 longitudinal + 8 lateral categories).
If the parsed intent is not recognizable within the closed decision set, $r_\text{consistency} = 0$ (conservative fallback).

**Step 3 — Label comparison across both axes:**
The two sets of labels are compared.
$r_\text{consistency} = 1$ if the reasoning-implied decision matches the trajectory-derived meta-actions for **both longitudinal AND lateral behavior**; otherwise $r_\text{consistency} = 0$.
...
**Demonstrated performance improvement from CoC:**
On challenging long-tail scenarios (0.5B backbone, route enabled), fine-tuning with CoC and trajectory jointly achieves minADE_6@6.4s of **0.868** versus **0.994** for trajectory-only fine-tuning — a **12.7% relative reduction in displacement error**.
Meta-action supervision alone (without the full causal reasoning chain) reaches 0.928, confirming that the structured causal grounding in CoC — not just the decision label — drives the improvement.
This is not merely a structural enforcement benefit; the causal trace actively improves the quality of the predicted trajectory.
...
This is partially enforced and partially an assumption, operating at two distinct levels.
At the decoder level, AR1 uses a unicycle dynamics model (acceleration $a^i$ + curvature $\kappa^i$ per waypoint) as its trajectory representation, so decoded waypoints are **kinematically feasible by construction** — physically impossible curvature changes or instantaneous velocity jumps cannot be produced.
At the evaluation level, AlpaSim tracks predicted trajectories with a **Model Predictive Controller** using a dynamically extended bicycle model, which provides a second enforcement layer for dynamic feasibility.
The residual assumption is that kinematic feasibility implies trackability: a unicycle-feasible trajectory can still be untrackable by a real vehicle at high speed (e.g., high curvature at high velocity exceeds the tire friction circle), and the paper does not quantify the gap between kinematic and dynamic feasibility under real vehicle conditions.
The decoder enforces the first constraint; the MPC absorbs the second; the assumption is that the MPC can always track the decoder's output without safety-relevant deviation.
