...
papers: "EMMA, AlphaDrive, Alpamayo-R1"
...
EMMA (random init): 0.37m, confirming Gemini pre-training weights drive the advantage. EMMA+: **0.29m**.
...
2. **Text coordinate representation is sufficient for safe planning.** Waypoints are generated token-by-token like prose. 
Small systematic errors may average out in L2 but still cause dangerous local failures (e.g. a stopping line).

3. **Self-supervised trajectory labels are sufficient to learn perception.** Only trained on ego waypoints, no perception labels. 
