...
transformer based model, and teach it to predict the next token based on any available data. Post-training is the stage where we train the model's reasoning capabilities. 
...
- **Objective**: The optimization signal (loss/reward) used to update the model.
- **Grounding**: The extent to which model representations are tied to external states (e.g., physical world, perception, or environment).

## Taxonomy of approaches

### Pre-GRPO Objective Shaping

Before GRPO, reasoning improvements were achieved using:
* RLHF (Reinforcement Learning from Human Feedback): uses human preference models as rewards
* DPO (Direct Preference Optimization): optimizes preferences without explicit RL

These approaches rely on human annotations or learned reward models.
...
1. **Objective Shaping for Reasoning Emergence:** This class of methods treats reasoning as an emergent property that can be induced through carefully designed optimization objectives rather than explicit symbolic structure. Reinforcement learning is used to bias the model toward generating longer, more coherent intermediate chains by rewarding correctness at the trajectory level. **GRPO** exemplifies this approach by replacing absolute reward estimation with group-relative advantage normalization, implicitly encouraging internal competition between candidate reasoning traces. The core assumption is that structured reasoning can be recovered purely through reward shaping over token sequences, without grounding in external state dynamics.
    ...
    Reward signals in such RL methods typically come from:

    * Verifiable correctness (e.g., math answers)
    * Programmatic evaluators (e.g., unit tests)
...
2. **Data & Pretraining Modality Scaling:** This line of work focuses on scaling reasoning capacity through domain-specialized pretraining data rather than changes to the objective. In this paradigm, reasoning improves as a function of exposure to large volumes of structured domain text. DeepSeekMath follows this strategy by aggressively curating large-scale mathematical corpora using Common Crawl and OpenWebMath and combining them with instruction tuning and RL. Here, reasoning is not explicitly enforced but statistically induced via data distribution shift toward problems that require multi-step abstraction. 
    ...
    Additionally, reasoning improvements are also supported by:
    * Chain-of-thought style data
    * Synthetic data generation (model-generated reasoning traces)
...
An open question is whether multimodal grounding provides fundamentally new reasoning signals or mainly acts as an auxiliary constraint. Current evidence suggests it improves robustness, but may not fully replace symbolic reasoning.
...
PPO uses an actor-critic model where a value function calculates the Advantage. Advantage (or Generalized Advantage Estimation / GAE) can be thought of as "How much better a specific action a<sub>t</sub> is compared to an average action (In PPO, the “average action” is approximated using the value function, which estimates expected future rewards from a state) considering the current state s<sub>t</sub>.

Reasoning improvements arise not only from distribution shift but also:

- Exposure to chain-of-thought style data
- Synthetic data generation (self-play, model-generated reasoning traces)
- Curriculum learning over difficulty

Thus, scaling data implicitly induces reasoning behaviors.
...
GRPO replaces value-function-based advantages with group-relative rewards, and moves KL regularization out of the reward and directly into the loss. Since, value function is a separate model that needs to be trained, skipping that step helps training the reinforcement learning part faster and more efficient. Group-relative normalization works by comparing samples within the same prompt, reducing variance and removing the need for a learned value function. It acts as a relative baseline rather than estimating future rewards.

While GRPO avoids value function estimation, variance is controlled through:

- Group sampling (relative comparisons)
- KL regularization
- Large batch sizes

This can be viewed as a relative baseline approximation instead of estimating future rewards explicitly. It works well in practice because it reduces variance through group comparisons and avoids training a separate value function, though it may still have higher variance than actor-critic methods.
...
**DeepSeek-R1** uses GRPO to train the model to reason on its own. It does this by answering and getting feedback on problems with verifiable answers, such as math and coding problems, or logic questions. 
The model is then prompted to answer these questions, as well as given a section using `<think><\think>`, where it can freely output any "thoughts" it has while trying to answer the question. 
The answer is ultimately compared to the true answer, and the model is given a reward if the answer is correct, and learns to improve its reasoning process and answers. 
The key innovation is that this was enough to produce and improve reasoning, compared to using expensive and noisy human preferences.
...
DeepSeek-R1 is a decoder-only Transformer built on DeepSeek-V3. It is a Mixture of Experts (MoE) model with up to 671 billion parameters (with multiple scales).

Mixture of Experts (MoE) models activate only a subset of parameters per token, allowing large capacity with reduced compute per forward pass.
...
It is trained on datasets such as OpenWebMath and filtered Common Crawl math corpora.
...
Hybrid Mamba-MLP-Transformer architecture [4]

It is trained on multimodal datasets combining vision, language, and task-specific physical reasoning data.

___

The **DeepSeekMath** base is a standard decoder-only Transformer (similar to LLaMA/Qwen style) with autoregressive next-token prediction prior to RL training. 

**DeepSeek-R1** is also a decoder-only Transformer. It is built upon the work of previous models by the group, specifically DeepSeek-V3. 
This model is a Mixture of Experts (or MoE models activate only a subset of parameters per token, enabling large model capacity with lower compute cost.
) model with 671 billion total parameters, as well as distilled versions of lower parameter counts. This basically means some feed forward layers get replaced with a sparse layer, usually consisting of many smaller feed forward networks, as well as a gating network which determines which "expert" is used at the time. This allows for faster inference and cheaper training, with only around 37 billion parameters active on average for the DeepSeek model.
DeepSeek-V3 also utilizes Multi-Head Latent Attention, a method the group came up with that fundamentally changes part of the Transformer model, allowing for dramatically lower KV-cache size, up to a factor of 57x, which also speeds up responses and allows for reasoning to be done faster. 
...
**Datasets used**
- DeepSeekMath: OpenWebMath, Common Crawl filtered math corpora
- DeepSeek-R1: 146k curated reasoning problems with verifiable answers
- Cosmos: multimodal datasets with vision-language-action alignment
...
| **Data volume emphasis** | Large pretrained LLM <br /> RL via reasoning prompts | Large pretrained model on 14.8 trillion tokens of internet scale text <br /> Additionally trained on ~146k prompts across Math/code/STEM/Logic and General problems  |Large vision data + curated task-specific multimodal corpora |
...
Cosmos-Reason1-7B, uses Qwen2.5-VL as the pre-trained model and the 

Cosmos-Reason1-56B, uses InternViT-300M-V2.5 as the vision encoder and Nemotron-H as the LLM backbone.
...
<figcaption>Hybrid Mamba-MLP architecture.</figcaption>
</figure>

## Interaction Mechanisms (How Models Fuse Data and/or Modalities)

Both DeepSeek-R1 and DeepSeekMath are trained on multilingual data, specifically both English and Chinese (since that is where the group is based).
One interesting consequence of the models being trained on both the English and Chinese language is that often the models will reason in both languages at the same time. This is more noticeable in DeepSeek-R1-Zero, which is purely trained using RL/GRPO, though occurs sometimes in the final DeepSeek-R1, which uses various training methods to try to eliminate this.

We weren't able to trigger the specific case of Chinese/English switching in the reasoning trace, but here's an example of it doing language switching in other languages, with the output apparently translating to "I see you have stopped with this task. Have some start first step, but we do not have the right to think flow now. Joc, I do not eat norpikar."

<img 
  src="https://raw.githubusercontent.com/arpg/vla-foundations/ce4119791c9f2edcd80f61c3f5ca43e0c687463e/content/textbook/audits/language_mixing.png" 
  alt="Obvious example of language switching, with (apparently) gibberish output when translated" 
  width="900" 
/>
...
- For input images, we convert the image into tiles and a low-resolution thumbnail image to maintain full context. Then these tokens are concatenated with interleaved tile IDs.
    ...
    The visual processing approach used here follows the LLaVA-NeXT paradigm, where high-resolution images are split into tiles alongside a global thumbnail to preserve both fine-grained and coarse features.

    However, more recent models such as Qwen2.5-VL improve upon this design with stronger vision encoders, better multimodal alignment, and more optimized training pipelines. As a result, they generally achieve better performance on vision-language reasoning tasks compared to LLaVA-style architectures.
...
<img 
  src="https://raw.githubusercontent.com/arpg/vla-foundations/762372b95e020009c1083aed4d17dbfef9366357/content/textbook/audits/Deepseek-arch.png"
  alt="DeepseekMath architecture"
  width="900" 
/>
...
By allowing the models to do reasoning freely without any specific restrictions (outside of keeping the process inside `<think><\think>` tokens), it allows the model to freely modify its reasoning process by itself. This self-evolution of reasoning also displays various interesting and useful properties. One such example is that, throughout the training process, the reasoning chains actually increase in length over time, which it isn't encouraged to do by the reward scheme. While the increased response length doesn't directly correspond with the increasing performance, it is an interesting correlation.

<figure>
  <img src="https://raw.githubusercontent.com/arpg/vla-foundations/audit/Soorej-ScalingAndReasoning/content/textbook/audits/deepseek_training.png" alt="Model performance and reasoning trace length through the training process" width="900" />
  <figcaption>Model performance vs reasoning trace length during training.</figcaption>
</figure>

This is purely an emergent behavior, as the model isn't explicitly encouraged to think for longer. This increased time "thinking" also leads to various emergent behaviors such as exploring alternate solutions, and the researchers note that the model begins to say "wait" in the later stages of training, and flagging some parts of its own reasoning as important or relevant.
...
* Reported cost of training of around $5-6 million, compared to the cost of 100s of millions or even billions required to train other SoTA LLMs.
  * Lead to the stock market going a bit haywire
  * Likely just the final training cost, not including research and development and testing costs, so still quite cheap but not as cheap as people initially believed.
...
4. Albert Gu and Tri Dao. Mamba: Linear-time sequence modeling with selective state spaces. In COLM, 2023.